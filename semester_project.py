...
# ---------- NEW: Temperature Trend Comparison ----------

# Yearly average temperature (all days)
yearly_avg_temp = (
    df.groupby("year")["temp_c"]
    .mean()
    .reset_index(name="avg_temp_all_days")
)

# Yearly average temperature (comfortable days only)
yearly_avg_comfort_temp = (
    comfortable.groupby("year")["temp_c"]
    .mean()
    .reset_index(name="avg_temp_comfortable_days")
)

# Merge for plotting
temp_trends = yearly_avg_temp.merge(
    yearly_avg_comfort_temp,
    on="year",
    how="left"
)

st.subheader(" Yearly Average Temperature Trends")

fig_temp_trend = go.Figure()

fig_temp_trend.add_trace(go.Scatter(
    x=temp_trends["year"],
    y=temp_trends["avg_temp_all_days"],
    mode="lines+markers",
    name="Avg Temp (All Days)"
))

fig_temp_trend.add_trace(go.Scatter(
    x=temp_trends["year"],
    y=temp_trends["avg_temp_comfortable_days"],
    mode="lines+markers",
    name="Avg Temp (Comfortable Days)"
))
...
yearly_avg_temp["avg_temp_all_days"] = yearly_avg_temp["avg_temp_all_days"].round(2)
...
summary_table = (
    yearly_total
    .merge(top_months[["year", "month_name", "monthly_days"]], on="year")
    .merge(yearly_avg_temp, on="year")
)
...
fig_table = go.Figure(data=[go.Table(
    header=dict(
        values=[
            "Year",
            "Total Comfortable Days",
            "Top Comfortable Month",
            "Days in Top Month",
            "Avg Temperature (°C)"
        ],
        fill_color="#1f2937",
    ...
    cells=dict(
        values=[
            ...
            summary_table["avg_temp_all_days"]
        ],
        align="center"
    )
)])
