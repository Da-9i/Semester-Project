...
# Yearly average temperature (FULL YEAR, not only comfortable days)
yearly_avg_temp = (
    df.groupby("year")["temp_c"]
    .mean()
    .reset_index(name="avg_temp_c")
)
yearly_avg_temp["avg_temp_c"] = yearly_avg_temp["avg_temp_c"].round(2)
